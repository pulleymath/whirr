...
async function renderRecorderSessionRealtime(ui: ReactElement) {
  const out = render(wrapRecorder(ui));
  await vi.waitFor(() => {
    expect(
...
describe("Recorder 세션 저장", () => {
  ...
  it("중지 시 스크립트 스냅샷이 비어 있지 않으면 saveSession을 한 번 호출한다", async () => {
    const { rerender } = await renderRecorderSessionRealtime(<Recorder />);
  ...
  it("onSessionSaved는 saveSession 성공 후 id와 함께 호출된다", async () => {
    ...
    const { rerender } = await renderRecorderSessionRealtime(
