...
| 변수                                      | 필수   | 설명                                                                                                                                                        |
| ----------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENAI_API_KEY`                          | 예     | OpenAI API(전사·Realtime·녹음 후 회의록 요약 등) 호출에 사용. **Production**에는 반드시 설정. **Preview**에서도 API를 쓰면 동일하게 또는 별도 키로 설정한다. |
| `STT_TOKEN_RATE_LIMIT_MAX`                | 아니오 | `POST /api/stt/token` 클라이언트당 허용 횟수 상한(기본값은 코드에 정의됨).                                                                                    |
| `STT_TOKEN_RATE_LIMIT_WINDOW_MS`          | 아니오 | 위 한도를 적용하는 시간 창(밀리초).                                                                                                                         |
| `MEETING_MINUTES_RATE_LIMIT_MAX`          | 아니오 | `POST /api/meeting-minutes`(녹음 후·상세에서 쓰는 회의록 요약) 클라이언트당 허용 횟수 상한.                                                                |
| `MEETING_MINUTES_RATE_LIMIT_WINDOW_MS`    | 아니오 | 위 한도를 적용하는 시간 창(밀리초).                                                                                                                         |

제품 기본 흐름의 **요약(회의록)** 은 `POST /api/meeting-minutes`를 쓴다. `POST /api/summarize`는 별도·보조 경로로 두어 있을 수 있으며, 배포 검증 시 둘 다 필요하면 각각 호출해 본다.
...
5. 선택적으로 `STT_TOKEN_RATE_LIMIT_*`·`MEETING_MINUTES_RATE_LIMIT_*`로 각 API 부하를 조절한다.
6. 배포 후 실제 도메인에서 녹음·전사·토큰 발급·회의록 요약 경로를 한 번씩 확인한다.
