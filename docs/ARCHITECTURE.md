...
### 2.2. 기본 구현: OpenAI Realtime Transcription

```
OpenAIRealtimeProvider implements TranscriptionProvider
  │
  ├── connect()  → API Route에서 에피메랄 토큰 발급 → OpenAI Realtime WSS (`intent=transcription`)
  ├── sendAudio() → 16kHz PCM을 24kHz로 리샘플 후 `input_audio_buffer.append`(base64)
  ├── stop()     → `input_audio_buffer.commit` 후 소켓 종료
  └── disconnect() → WebSocket 정리
```

레거시로 `AssemblyAIRealtimeProvider`(`assemblyai.ts`)를 유지한다. 훅에서 `createProvider`·`useAssemblyAiPcmFraming`으로 조합할 수 있다.
...
| 경로             | 메서드 | 역할                                                                                           | 응답                                     |
| ---------------- | ------ | ---------------------------------------------------------------------------------------------- | ---------------------------------------- |
| `/api/stt/token` | POST   | OpenAI `transcription_sessions` 에피메랄 토큰 발급(클라이언트 키별 메모리 기반 요청 빈도 제한) | `{ "token": "..." }` 또는 `429`/`502` 등 |
...
Browser                    Next.js API Route                 OpenAI API
  ...
  │                              │   Authorization: Bearer (서버 API 키)──→│
  │                              │←── { client_secret: { value } } ─────────│
  │←── { "token": value } ───────│                              │
  │                              │                              │
  │── WSS /v1/realtime?intent=transcription (서브프로토콜 인증) ──────────────→│
```

업스트림은 `https://api.openai.com`이다.
...
### 5.1. OpenAI Realtime Transcription (기본 Provider)

**연결**: `wss://api.openai.com/v1/realtime?intent=transcription` — WebSocket 서브프로토콜 `realtime`, `openai-insecure-api-key.{ephemeral_token}`.

연결 후 `transcription_session.update`로 `input_audio_format: pcm16`(24kHz mono s16le), `input_audio_transcription.model: gpt-4o-mini-transcribe-2025-12-15`, `language: ko`, 서버 VAD·노이즈 리덕션을 설정한다.

#### 브라우저 → OpenAI

| 메시지                      | 형식                                   | 설명                                           |
| --------------------------- | -------------------------------------- | ---------------------------------------------- |
| `input_audio_buffer.append` | JSON, `audio`는 **24kHz** PCM의 base64 | Provider가 16kHz Worklet 출력을 리샘플 후 전송 |
| `input_audio_buffer.commit` | JSON                                   | 녹음 종료 시 잔여 버퍼 확정                    |

#### OpenAI → 브라우저

| 메시지 타입                                             | 설명                      |
| ------------------------------------------------------- | ------------------------- |
| `conversation.item.input_audio_transcription.delta`     | 부분 전사 (`delta`)       |
| `conversation.item.input_audio_transcription.completed` | 확정 전사 (`transcript`)  |
| `error`                                                 | 오류 — UI용 문구로 정규화 |

### 5.2. AssemblyAI Universal Streaming v3 (레거시 Provider)

**연결**: `wss://streaming.assemblyai.com/v3/ws?token={temp_token}&sample_rate=16000&format_turns=true&speech_model=...` (`NEXT_PUBLIC_ASSEMBLYAI_STREAMING_WS_ORIGIN` 등으로 조정 가능)

#### 브라우저 → AssemblyAI

| 메시지        | 형식                           | 설명                                |
| ------------- | ------------------------------ | ----------------------------------- |
| 오디오 데이터 | **바이너리** PCM `ArrayBuffer` | 16kHz 모노 PCM 청크(Worklet과 동일) |
| 세션 종료     | JSON `{"type": "Terminate"}`   | 전사 종료 신호                      |

#### AssemblyAI → 브라우저

| 메시지 타입            | 형식(요지)                  | 설명                                       |
| ---------------------- | --------------------------- | ------------------------------------------ |
| `Begin`                | 세션 시작 메타데이터        | 연결 확립                                  |
| `Turn`                 | `transcript`, `end_of_turn` | `end_of_turn: false` → 부분, `true` → 확정 |
| `Termination`          | 세션 종료                   | Provider가 소켓을 닫음                     |
| `Error` / `error` 필드 | 오류                        | `STT_PROVIDER_ERROR`로 정규화              |

### 5.3. 앱 내부 이벤트 (Provider 추상화 후)
...
[AudioWorklet] ─ PCM 16kHz mono
   │
   ▼
[STT Provider] ─ 16→24k 리샘플 · base64 ─→ [OpenAI Realtime WSS]
                                        │
                      부분/확정 텍스트 ←──┘
                          │
                          ▼
...
│   │   │   ├── assemblyai.ts      # AssemblyAI 구현체(레거시)
│   │   │   ├── openai-realtime.ts # OpenAI Realtime Transcription(기본)
│   │   │   └── index.ts           # 팩토리 + 타입 re-export
...
│
├── .env.local                     # OPENAI_API_KEY (서버 전용)
├── next.config.ts
├── package.json
├── tsconfig.json
├── PRD.md
...
| 변수명           | 위치                         | 설명                                     |
| ---------------- | ---------------------------- | ---------------------------------------- |
| `OPENAI_API_KEY` | Vercel 환경 변수 (서버 전용) | OpenAI API 키(토큰 라우트·업스트림 전용) |
...
4. **Environment Variables**에 `OPENAI_API_KEY`를 추가한다(Production / Preview 필요 시 동일하게).
...
외부 STT(OpenAI·AssemblyAI 등)에서 자체 호스팅(Whisper/faster-whisper 등)으로 전환할 경우:
