...
```
Browser                    Next.js API Route            AssemblyAI Streaming
  │                              │                          │
  │── POST /api/stt/token ──────→│                          │
  │                              │── GET /v3/token?expires_in_seconds=…│
  │                              │   (Authorization: API Key)──────────→│
  │                              │←── { "token": "..." } ──────────────│
  │←── { "token": "..." } ──────│                          │
  │                              │                          │
  │── WSS /v3/ws (쿼리 token·sample_rate 등) ─────────────────────────→│
```

기본 업스트림 호스트는 `https://streaming.assemblyai.com`이며, `ASSEMBLYAI_STREAMING_API_BASE`로 EU 등 다른 리전을 지정할 수 있다.
...
### 5.1. AssemblyAI Universal Streaming v3 (현재 Provider)

**연결**: `wss://streaming.assemblyai.com/v3/ws?token={temp_token}&sample_rate=16000&format_turns=true&speech_model=universal-streaming-multilingual` (기본값; `NEXT_PUBLIC_ASSEMBLYAI_STREAMING_WS_ORIGIN`·`NEXT_PUBLIC_ASSEMBLYAI_SPEECH_MODEL`로 조정 가능)
